/**
 * Données du CST — Phase 1 (contenu réel issu du document institutionnel).
 *
 * Ces données sont consommées uniquement via lib/api.ts.
 * Lors du branchement Django, ce fichier devient inactif.
 */
import type {
  Category,
  DocumentItem,
  Session,
  Member,
  NewsItem,
  GalleryAlbum,
  UsefulLink,
} from "./types";

/** Exemple de fichier téléchargeable (placeholder). */
export const SAMPLE_FILE = "/documents/exemple.pdf";

// ---------------------------------------------------------------
// Catégories
// ---------------------------------------------------------------
export const CATEGORIES: Category[] = [
  { id: 1, slug: "decisions", name: "Décisions", kind: "decision", },
  { id: 2, slug: "rapports", name: "Rapports", kind: "rapport", },
  { id: 3, slug: "pv", name: "Procès-verbaux", kind: "proces_verbal", },
  { id: 4, slug: "communiques", name: "Communiqués", kind: "communique", },
  { id: 5, slug: "textes", name: "Textes consolidés", kind: "texte_consolide", },
  { id: 6, slug: "vulgarisation", name: "Documents de vulgarisation", kind: "document", },
];

// ---------------------------------------------------------------
// Documents (contenu réel — références fictives)
// ---------------------------------------------------------------
export const DOCUMENTS: DocumentItem[] = [
  {
    id: 1, slug: "constitution-consolidee", reference: "CST/TXT/2026-001",
    title: "Constitution consolidée de l'Église du Christianisme Céleste",
    summary: "Version consolidée de la Constitution issue des travaux de relecture et d'harmonisation menés par le CST et ses commissions.",
    kind: "document", categorySlug: "textes", date: "2026-04-30",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 2456000, downloads: 342,
    featured: true, status: "publie",
  },
  {
    id: 2, slug: "reglement-interieur-consolide", reference: "CST/TXT/2026-002",
    title: "Règlement intérieur consolidé",
    summary: "Règlement intérieur de l'Église, élaboré et consolidé par les commissions du CST pour encadrer le fonctionnement des organes.",
    kind: "document", categorySlug: "textes", date: "2026-04-30",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 1890000, downloads: 278,
    featured: true, status: "publie",
  },
  {
    id: 3, slug: "rapport-final-cst", reference: "CST/RAP/2026-001",
    title: "Rapport final du Conseil Supérieur de Transition",
    summary: "Synthèse des travaux des sessions, conclusions des commissions, propositions de textes, décisions et recommandations. Remis le 30 avril 2026 au Sofitel Cotonou.",
    kind: "rapport", categorySlug: "rapports", date: "2026-04-30",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 5200000, downloads: 456,
    featured: true, status: "publie",
  },
  {
    id: 4, slug: "acte-installation-cst", reference: "CST/DEC/2025-001",
    title: "Acte d'installation du CST",
    summary: "Acte officiel portant installation du Conseil Supérieur de Transition, signé à Cotonou le 26 avril 2025 sous la facilitation du Président de la République du Bénin.",
    kind: "decision", categorySlug: "decisions", date: "2025-04-26",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 890000, downloads: 312,
    featured: true, status: "publie",
  },
  {
    id: 5, slug: "acte-creation-csmo", reference: "CST/DEC/2026-003",
    title: "Acte portant mise en place du CSMo",
    summary: "Décision instituant le Conseil Supérieur de Mise en Œuvre (CSMo) pour assurer l'application des textes et recommandations du CST. Entré en vigueur le 23 mai 2026.",
    kind: "decision", categorySlug: "decisions", date: "2026-05-23",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 678000, downloads: 201,
    featured: false, status: "publie",
  },
  {
    id: 6, slug: "harmonisation-grades", reference: "CST/TXT/2026-003",
    title: "Tableau d'harmonisation des grades",
    summary: "Harmonisation des grades entre les espaces francophone et anglophone : leaders, visionnaires, Allagba et Mamans.",
    kind: "annexe", categorySlug: "textes", date: "2026-03-27",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 456000, downloads: 167,
    featured: false, status: "publie",
  },
  {
    id: 7, slug: "organigramme-mondial", reference: "CST/TXT/2026-004",
    title: "Organigramme mondial de la gouvernance",
    summary: "Structure des organes mondiaux : Pasteur, Synode, Conseil Pastoral, Conseil d'Administration, Comité Exécutif, Bureau Exécutif, Conclave et départements.",
    kind: "document", categorySlug: "textes", date: "2026-04-17",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 1100000, downloads: 234,
    featured: false, status: "publie",
  },
  {
    id: 8, slug: "pv-9e-session", reference: "CST/PV/2026-009",
    title: "Procès-verbal de la 9e session",
    summary: "Finalisation des travaux, consolidation des conclusions et préparation du rapport final. Tenue les 16 et 17 avril 2026.",
    kind: "proces_verbal", categorySlug: "pv", date: "2026-04-17",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 780000, downloads: 98,
    featured: false, status: "publie",
  },
  {
    id: 9, slug: "communique-remise-rapport", reference: "CST/COM/2026-001",
    title: "Communiqué — Remise du rapport final",
    summary: "Communiqué officiel annonçant la remise du rapport final du CST le 30 avril 2026 au Sofitel Cotonou, salle Les Collines.",
    kind: "communique", categorySlug: "communiques", date: "2026-04-30",
    fileUrl: SAMPLE_FILE, fileType: "pdf", fileSize: 345000, downloads: 156,
    featured: false, status: "publie",
  },
];

// ---------------------------------------------------------------
// Sessions (contenu réel)
// ---------------------------------------------------------------
export const SESSIONS: Session[] = [
  {
    id: 1, slug: "installation-cst", number: 0,
    title: "Installation officielle du CST", theme: "Mise en place du Conseil",
    summary: "Installation officielle du Conseil Supérieur de Transition à Cotonou, sous la facilitation du Président de la République du Bénin.",
    startDate: "2025-04-26", endDate: "2025-04-26",
    location: "Cotonou", imageUrls: ["/images/session-1.svg"],
    documentSlugs: ["acte-installation-cst"], status: "publie",
  },
  {
    id: 2, slug: "5e-session", number: 5,
    title: "5e session du CST", theme: "Réunification et orientations institutionnelles",
    summary: "Poursuite des travaux de réunification, examen des textes et préparation des orientations institutionnelles.",
    startDate: "2025-10-28", endDate: "2025-10-31",
    location: "Cotonou", imageUrls: ["/images/session-2.svg"],
    documentSlugs: [], status: "publie",
  },
  {
    id: 3, slug: "6e-session", number: 6,
    title: "6e session du CST", theme: "Textes fondamentaux et consensus",
    summary: "Approfondissement des textes fondamentaux, concertations et recherche de consensus entre les différentes composantes.",
    startDate: "2025-11-24", endDate: "2025-11-29",
    location: "Cotonou", imageUrls: ["/images/session-3.svg"],
    documentSlugs: [], status: "publie",
  },
  {
    id: 4, slug: "7e-session", number: 7,
    title: "7e session du CST", theme: "Gouvernance et consolidation des textes",
    summary: "Examen des propositions de gouvernance et poursuite de la consolidation des textes de l'Église.",
    startDate: "2026-01-08", endDate: "2026-01-10",
    location: "Cotonou", imageUrls: [],
    documentSlugs: [], status: "publie",
  },
  {
    id: 5, slug: "8e-session", number: 8,
    title: "8e session du CST", theme: "Organes et déploiement diocésain",
    summary: "Examen des modalités de constitution des organes et du programme de déploiement des groupes CST dans les diocèses.",
    startDate: "2026-02-24", endDate: "2026-02-26",
    location: "Cotonou", imageUrls: [],
    documentSlugs: [], status: "publie",
  },
  {
    id: 6, slug: "9e-session", number: 9,
    title: "9e session du CST", theme: "Finalisation et rapport final",
    summary: "Finalisation des travaux, consolidation des conclusions et préparation du rapport final du CST.",
    startDate: "2026-04-16", endDate: "2026-04-17",
    location: "Cotonou", imageUrls: [],
    documentSlugs: ["pv-9e-session", "rapport-final-cst"], status: "publie",
  },
];

// ---------------------------------------------------------------
// Membres identifiés
// ---------------------------------------------------------------
export const MEMBERS: Member[] = [
  // Sacré Collège
  {
    id: 1,
    slug: "emmanuel-mf-oshoffa",
    fullName: "Emmanuel M.F. OSHOFFA",
    role: "Révérend Pasteur",
    responsibility: "Sacré Collège · Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/emf-oshoffa.jpg",
    bio: "Pays de provenance : Nigeria",
    order: 1,
    status: "publie",
  },
  {
    id: 2,
    slug: "benneth-a-adeogun",
    fullName: "Benneth A. ADEOGUN",
    role: "Révérend Pasteur",
    responsibility: "Sacré Collège · Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/adeogun.jpg",
    bio: "Pays de provenance : Bénin",
    order: 2,
    status: "publie",
  },

  // Secrétariat exécutif
  {
    id: 3,
    slug: "bertin-bada",
    fullName: "Bertin BADA",
    role: "Coordonnateur Général",
    responsibility: "Secrétariat exécutif · Membre de Commission N°1 · Diocèse de désignation : PR",
    photoUrl: "/images/membres/bada.jpg",
    bio: "Pays de provenance : Bénin",
    order: 3,
    status: "publie",
  },
  {
    id: 4,
    slug: "andre-amoussou",
    fullName: "André AMOUSSOU",
    role: "1er Coordonnateur Adjoint",
    responsibility: "Secrétariat exécutif · Membre des Commissions N°1 et N°3 · Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/amoussou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 4,
    status: "publie",
  },
  {
    id: 5,
    slug: "senou-jean-kokoye",
    fullName: "Sènou Jean KOKOYE",
    role: "2ème Coordonnateur",
    responsibility: "Secrétariat exécutif · Membre de Commission N°2 · Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/kokoye.jpg",
    bio: "Pays de provenance : Bénin",
    order: 5,
    status: "publie",
  },
  {
    id: 6,
    slug: "marcellin-zannou",
    fullName: "Marcellin ZANNOU",
    role: "Rapporteur Général",
    responsibility: "Secrétariat exécutif · Membre des Commissions N°1 et N°3 · Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/zannou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 6,
    status: "publie",
  },
  {
    id: 7,
    slug: "olatoscho-oschoffa",
    fullName: "Olatoscho OSCHOFFA",
    role: "Rapporteur Adjoint",
    responsibility: "Secrétariat exécutif · Membre de Commission N°2 · Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/olatosho.jpg",
    bio: "Pays de provenance : Nigeria",
    order: 7,
    status: "publie",
  },
  {
    id: 8,
    slug: "kouame-justin-ackah",
    fullName: "Kouamé Justin ACKAH",
    role: "Trésorier Général",
    responsibility: "Secrétariat exécutif · Membre des Commissions N°1 et N°3 · Diocèse de désignation : Côte d’Ivoire",
    photoUrl: "/images/membres/ackah.jpg",
    bio: "Pays de provenance : Côte d’Ivoire",
    order: 8,
    status: "publie",
  },
  {
    id: 9,
    slug: "komlan-midodji",
    fullName: "Komlan MIDODJI",
    role: "Trésorier Adjoint",
    responsibility: "Secrétariat exécutif · Membre de Commission N°1 · Diocèse de désignation : Diocèse de Bellow",
    photoUrl: "/images/membres/midodji.jpg",
    bio: "Pays de provenance : Togo",
    order: 9,
    status: "publie",
  },

  // Membres du CST
  {
    id: 10,
    slug: "hayacinthe-sarassoro",
    fullName: "Hayacinthe SARASSORO",
    role: "Président de Commission N°1",
    responsibility: "Membre du CST · Diocèse de désignation : Côte d’Ivoire",
    photoUrl: "/images/membres/sarassoro.jpg",
    bio: "Pays de provenance : Côte d’Ivoire",
    order: 10,
    status: "publie",
  },
  {
    id: 11,
    slug: "rodrigue-chabi",
    fullName: "Rodrigue CHABI",
    role: "Président de Commission N°2",
    responsibility: "Membre du CST · Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/chabi.jpg",
    bio: "Pays de provenance : Bénin",
    order: 11,
    status: "publie",
  },
  {
    id: 12,
    slug: "gabriel-soumaho",
    fullName: "Gabriel SOUMAHO",
    role: "Président de Commission N°3",
    responsibility: "Membre du CST · Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/soumaho.jpg",
    bio: "Pays de provenance : Bénin",
    order: 12,
    status: "publie",
  },
  {
    id: 13,
    slug: "noel-djossou",
    fullName: "Noël DJOSSOU",
    role: "Membre CST",
    responsibility: "Membre de Commission N°1 · Diocèse de désignation : Diocèse de Djossou",
    photoUrl: "/images/membres/djossou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 13,
    status: "publie",
  },
  {
    id: 14,
    slug: "frank-okhueleigbe",
    fullName: "Frank OKHUELEIGBE",
    role: "Membre CST",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membre.svg",
    bio: "Pays de provenance : Nigeria",
    order: 14,
    status: "publie",
  },
  {
    id: 15,
    slug: "moses-oluwole-ajao",
    fullName: "Moses Oluwole AJAO",
    role: "Membre CST",
    responsibility: "Membre de Commission N°2 · Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/ajao.jpg",
    bio: "Pays de provenance : Nigeria",
    order: 15,
    status: "publie",
  },

  // Membres des commissions
  {
    id: 16,
    slug: "joel-edgar-alexis-thognon",
    fullName: "Joël Edgar Alexis THOGNON",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Diocèse de Shobwalé",
    photoUrl: "/images/membres/thognon.jpg",
    bio: "Pays de provenance : France",
    order: 16,
    status: "publie",
  },
  {
    id: 17,
    slug: "noel-etienne-pierre-tahet",
    fullName: "Noël Etienne Pierre TAHET",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Côte d’Ivoire",
    photoUrl: "/images/membres/tahet.jpg",
    bio: "Pays de provenance : Côte d’Ivoire",
    order: 17,
    status: "publie",
  },
  {
    id: 18,
    slug: "gnanza-kouyo",
    fullName: "Gnanza KOUYO",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Côte d’Ivoire",
    photoUrl: "/images/membres/kouyo.jpg",
    bio: "Pays de provenance : Côte d’Ivoire",
    order: 18,
    status: "publie",
  },
  {
    id: 19,
    slug: "konan-jean-raphael-ezechiel-brou",
    fullName: "Konan Jean Raphaël Ezéchiel BROU",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Côte d’Ivoire",
    photoUrl: "/images/membres/brou-konan.jpg",
    bio: "Pays de provenance : Côte d’Ivoire",
    order: 19,
    status: "publie",
  },
  {
    id: 20,
    slug: "rufus-bolawa-adeoba",
    fullName: "Rufus Bolawa ADEOBA",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/adeoba.jpg",
    bio: "Pays de provenance : Amérique du Nord (Canada)",
    order: 20,
    status: "publie",
  },
  {
    id: 21,
    slug: "guy-roger-ozouany",
    fullName: "Guy Roger OZOUANY",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/ozouany.jpg",
    bio: "Pays de provenance : Gabon",
    order: 21,
    status: "publie",
  },
  {
    id: 22,
    slug: "emmanuel-ayoola-osho",
    fullName: "Emmanuel Ayoola OSHO",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/osho.jpg",
    bio: "Pays de provenance : Royaume-Uni",
    order: 22,
    status: "publie",
  },
  {
    id: 23,
    slug: "enoch-miatabouna",
    fullName: "Enoch MIATABOUNA",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/miatabouna.jpg",
    bio: "Pays de provenance : Congo-Brazzaville",
    order: 23,
    status: "publie",
  },
  {
    id: 24,
    slug: "paul-kanyi",
    fullName: "Paul KANYI",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/kanyi.jpg",
    bio: "Pays de provenance : Ghana",
    order: 24,
    status: "publie",
  },
  {
    id: 25,
    slug: "theodule-ricardo-adetonah",
    fullName: "Théodule Ricardo ADETONAH",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/adetonah.jpg",
    bio: "Pays de provenance : Cameroun",
    order: 25,
    status: "publie",
  },
  {
    id: 26,
    slug: "jacob-salomon-kuti",
    fullName: "Jacob Salomon KUTI",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/kuti.jpg",
    bio: "Pays de provenance : Nigeria",
    order: 26,
    status: "publie",
  },
  {
    id: 27,
    slug: "jean-marie-gbassi",
    fullName: "Jean-Marie GBASSI",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/gbassi.jpg",
    bio: "Pays de provenance : Bénin",
    order: 27,
    status: "publie",
  },
  {
    id: 28,
    slug: "jacob-oshoffa",
    fullName: "Jacob OSHOFFA",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/oshoffa-jacob.jpg",
    bio: "Pays de provenance : Bénin",
    order: 28,
    status: "publie",
  },
  {
    id: 29,
    slug: "moise-amoussou",
    fullName: "Moïse AMOUSSOU",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/amoussou-moise.jpg",
    bio: "Pays de provenance : Bénin",
    order: 29,
    status: "publie",
  },
  {
    id: 30,
    slug: "etienne-ahonahin",
    fullName: "Etienne AHONAHIN",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/ahonahin.jpg",
    bio: "Pays de provenance : Bénin",
    order: 30,
    status: "publie",
  },
  {
    id: 31,
    slug: "appolinaire-aholouye",
    fullName: "Appolinaire AHOLOUYÈ",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/aholouye.jpg",
    bio: "Pays de provenance : Bénin",
    order: 31,
    status: "publie",
  },
  {
    id: 32,
    slug: "michel-adjaka",
    fullName: "Michel ADJAKA",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/adjaka.jpg",
    bio: "Pays de provenance : Bénin",
    order: 32,
    status: "publie",
  },
  {
    id: 33,
    slug: "kadoke-salomon-abou",
    fullName: "Kadoké Salomon ABOU",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/abou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 33,
    status: "publie",
  },
  {
    id: 34,
    slug: "justin-odjo",
    fullName: "Justin ODJO",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/odjo-justin.jpg",
    bio: "Pays de provenance : Bénin",
    order: 34,
    status: "publie",
  },
  {
    id: 35,
    slug: "jean-soglohoun",
    fullName: "Jean SOGLOHOUN",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/soglohoun.jpg",
    bio: "Pays de provenance : Bénin",
    order: 35,
    status: "publie",
  },
  {
    id: 36,
    slug: "alexandre-biaou",
    fullName: "Alexandre BIAOU",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/biaou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 36,
    status: "publie",
  },
  {
    id: 37,
    slug: "prosper-dah-guedenon",
    fullName: "Prosper DAH GUEDENON",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/guedenon.jpg",
    bio: "Pays de provenance : Bénin",
    order: 37,
    status: "publie",
  },
  {
    id: 38,
    slug: "kinnenon-zacharie",
    fullName: "Kinnenon ZACHARIE",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Diocèse de Djossou",
    photoUrl: "/images/membres/zacharie-kinnenon.jpg",
    bio: "Pays de provenance : Bénin",
    order: 38,
    status: "publie",
  },
  {
    id: 39,
    slug: "adeola-theodore-amouzou",
    fullName: "Adeola Théodore AMOUZOU",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : Nigeria",
    photoUrl: "/images/membres/amouzou.jpg",
    bio: "Pays de provenance : Togo",
    order: 39,
    status: "publie",
  },
  {
    id: 40,
    slug: "tetemark-raymond-agbofin",
    fullName: "Tétémark Raymond AGBOFIN",
    role: "Membre de Commission N°2",
    responsibility: "Diocèse de désignation : Bénin",
    photoUrl: "/images/membres/agbofin.jpg",
    bio: "Pays de provenance : Togo",
    order: 40,
    status: "publie",
  },
  {
    id: 41,
    slug: "leandre-gbaguidi",
    fullName: "Léandre GBAGUIDI",
    role: "Membre de Commission N°3",
    responsibility: "Diocèse de désignation : PR",
    photoUrl: "/images/membres/gbaguidi.jpg",
    bio: "Pays de provenance : Bénin",
    order: 41,
    status: "publie",
  },
  {
    id: 42,
    slug: "roland-agossou",
    fullName: "Roland AGOSSOU",
    role: "Membre de Commission N°1",
    responsibility: "Diocèse de désignation : PR",
    photoUrl: "/images/membres/agossou.jpg",
    bio: "Pays de provenance : Bénin",
    order: 42,
    status: "publie",
  },
];

// ---------------------------------------------------------------
// Actualités
// ---------------------------------------------------------------
export const NEWS: NewsItem[] = [
  {
    id: 1, slug: "remise-rapport-final",
    title: "Remise du rapport final du CST",
    excerpt: "Le rapport final du Conseil Supérieur de Transition a été officiellement remis le 30 avril 2026 au Sofitel Cotonou.",
    content: "Le rapport final du Conseil Supérieur de Transition a été officiellement remis le 30 avril 2026 à 12 h 30, au Sofitel Cotonou, dans la salle Les Collines.\n\nCe rapport a synthétisé les travaux des sessions, les conclusions des commissions, les résultats du dialogue, les propositions de textes, les principales décisions, les recommandations, les mesures nécessaires à la mise en œuvre et les perspectives institutionnelles de l'Église.\n\nLa remise du rapport final a marqué la clôture de la phase principale de transition conduite par le CST et l'ouverture de la phase de mise en œuvre.",
    imageUrl: "/images/news/remise-rapport-final.jpg",
    imageAlt: "Remise officielle du rapport final du Conseil Supérieur de Transition",
    featured: false, date: "2026-04-30",
    relatedDocumentSlugs: ["rapport-final-cst", "communique-remise-rapport"],
    status: "publie",
  },
  {
    id: 2, slug: "installation-csmo",
    title: "Installation officielle du CSMo",
    excerpt: "Le Conseil Supérieur de Mise en Œuvre a tenu sa première session du 2 au 5 juin 2026 à Cotonou.",
    content: "Le Conseil Supérieur de Mise en Œuvre a tenu sa première session du 2 au 5 juin 2026 à Cotonou. La cérémonie d'installation de ses membres s'est déroulée le 4 juin 2026, au Sofitel Cotonou, dans la salle Le Dôme.\n\nCette nouvelle phase doit permettre la vulgarisation des textes, l'installation des organes, le déploiement dans les diocèses, la mise en œuvre de la cartographie ecclésiale, la préparation du Synode et du conclave, la modernisation administrative, la digitalisation et le suivi des décisions issues du CST.",
    imageUrl: "/images/news/installation-csmo.jpg",
    imageAlt: "Installation officielle du Conseil Supérieur de Mise en Œuvre",
    featured: true, date: "2026-06-04",
    relatedDocumentSlugs: ["acte-creation-csmo"],
    status: "publie",
  },
  {
    id: 3, slug: "mission-cote-divoire",
    title: "Séances d'information en Côte d'Ivoire",
    excerpt: "Du 24 au 27 mars 2026, des séances d'information ont été menées en Côte d'Ivoire pour présenter l'avancement des travaux.",
    content: "Du 24 au 27 mars 2026, des séances d'information ont été menées en Côte d'Ivoire. Cette activité s'inscrivait dans la volonté d'informer les responsables et fidèles, de présenter l'évolution des travaux, de favoriser l'appropriation du processus et de renforcer la dimension internationale de la réunification.",
    imageUrl: "/images/news/mission-cote-divoire.jpg",
    imageAlt: "Séances d’information sur le processus CST et CSMo en Côte d’Ivoire",
    featured: false, date: "2026-03-27",
    relatedDocumentSlugs: [],
    status: "publie",
  },
  {
    id: 4,
    slug: "alerte-info-communique",
    title: "Communiqué officiel",
    excerpt:
      "Retrouvez les dernières informations officielles relatives au processus de mise en œuvre.",
    content:
      "Un communiqué officiel a été publié dans le cadre du processus de mise en œuvre.\n\nLe document intégral est disponible ci-dessous.",
    imageUrl: "/images/home/hero-cst.jpg",
    imageAlt: "Communiqué officiel du CSMo",
    featured: false,
    date: "2026-09-16",

    homeSlot: "alert_info",

    attachment: {
      type: "pdf",
      url: SAMPLE_FILE,
      label: "Communiqué officiel — PDF",
    },

    relatedDocumentSlugs: [],
    status: "publie",
  },
  {
    id: 5,
    slug: "prochain-evenement-csmo",
    title: "Prochain rendez-vous du processus",
    excerpt:
      "Découvrez les informations pratiques et le programme du prochain temps fort.",
    content:
      "Un nouveau rendez-vous est prévu dans le cadre de la mise en œuvre du processus.\n\nRetrouvez sur cette page le programme et les informations utiles.",
    imageUrl: "/images/home/next-event.jpg",
    imageAlt: "Prochain événement du CSMo",
    featured: true,
    date: "2026-09-16",

    homeSlot: "upcoming_event",
    eventDate: "2026-09-23",

    attachment: {
      type: "image",
      url: "/images/home/next-event.jpg",
      label: "Flyer de l'événement",
    },

    relatedDocumentSlugs: [],
    status: "publie",
  },
];

// ---------------------------------------------------------------
// Albums & Liens utiles
// ---------------------------------------------------------------
export const ALBUMS: GalleryAlbum[] = [
  {
    id: 1,
    slug: "installation-cst-26-avril-2025",
    title: "Installation du CST — 26 avril 2025",
    description: "Cérémonie d'installation officielle à Cotonou.",
    date: "2025-04-26",
    coverUrl: "/images/gallery/installation-cst-01.jpg",
    images: [
      {
        id: 1,
        title: "Installation du CST — photographie 1",
        imageUrl: "/images/gallery/installation-cst-01.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1600,
        height: 1067,
      },
      {
        id: 2,
        title: "Installation du CST — photographie 2",
        imageUrl: "/images/gallery/installation-cst-02.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1200,
        height: 1600,
      },
      {
        id: 3,
        title: "Installation du CST — photographie 3",
        imageUrl: "/images/gallery/installation-cst-03.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1600,
        height: 1200,
      },
      {
        id: 4,
        title: "Installation du CST — photographie 4",
        imageUrl: "/images/gallery/installation-cst-04.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1200,
        height: 1500,
      },
      {
        id: 5,
        title: "Installation du CST — photographie 5",
        imageUrl: "/images/gallery/installation-cst-05.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1600,
        height: 1000,
      },
      {
        id: 6,
        title: "Installation du CST — photographie 6",
        imageUrl: "/images/gallery/installation-cst-06.jpg",
        alt: "Photographie de la cérémonie d'installation du CST",
        width: 1200,
        height: 1600,
      },
    ],
    status: "publie",
  },
];

export const USEFUL_LINKS: UsefulLink[] = [
  { id: 1, label: "Église du Christianisme Céleste", url: "https://www.ecc-international.org" },
];
